...
impl Buffer {
    pub fn new(w: u16, h: u16) -> Result<Self> {
        ...
        Ok(Buffer { w, h, display: vec![Cell::default(); sz], back: vec![Cell::default(); sz] })
    }
    ...
    pub fn write_char(&mut self, ch: char, col: u16, row: u16, fg: Color, bg: Color) {
...
mod buf_test {
    ...
    fn h_resize_test() {
        ...
        for d in data.iter() {
            scr.write_char(d.c, d.x, d.y, Color::White, Color::Black);
        }
        scr.flip();
